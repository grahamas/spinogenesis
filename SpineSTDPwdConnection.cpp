...
void SpineSTDPwdConnection::init(AurynWeight lambda, AurynWeight maxweight)
{
...
void SpineSTDPwdConnection::compute_fudge_factors()
{
...
void SpineSTDPwdConnection::init_shortcuts()
{
...
void SpineSTDPwdConnection::finalize() {
	DuplexConnection::finalize();
	init_shortcuts();
}

void SpineSTDPwdConnection::free()
{
}

SpineSTDPwdConnection::SpineSTDPwdConnection(SpikingGroup * source, NeuronGroup * destination, TransmitterType transmitter) : DuplexConnection(source, destination, transmitter)
{
}

SpineSTDPwdConnection::SpineSTDPwdConnection(SpikingGroup * source, NeuronGroup * destination,
		const char * filename,
		AurynWeight lambda,
		...
		name)
{
	init(lambda, maxweight);
	if ( name.empty() )
		set_name("SpineSTDPwdConnection");
	init_shortcuts();
}

SpineSTDPwdConnection::~SpineSTDPwdConnection()
{
...
void SpineSTDPwdConnection::propagate_forward()
{
...
void SpineSTDPwdConnection::propagate_backward()
{
...
void SpineSTDPwdConnection::set_alpha(AurynWeight a)
{
	param_alpha = a;
	compute_fudge_factors();
...
void SpineSTDPwdConnection::set_lambda(AurynWeight l)
{
	param_lambda = l;
	compute_fudge_factors();
...
void SpineSTDPwdConnection::set_mu_plus(AurynWeight m)
{
	param_mu_plus = m;
	compute_fudge_factors();
...
void SpineSTDPwdConnection::set_mu_minus(AurynWeight m)
{
	param_mu_minus = m;
	compute_fudge_factors();
...
void SpineSTDPwdConnection::set_max_weight(AurynWeight wmax)
{
